...
class TestRedshift extends Validator {
  ...
  testRedshift () {
    ...
    this.validateIdentity('DATEDIFF(days, a, b)', 'DATEDIFF(DAY, a, b)');
    this.validateAll(
      'DATEDIFF(\'day\', a, b)',
      {
        write: {
  ...
  testIdentity () {
    this.validateIdentity('SELECT GETBIT(FROM_HEX(\'4d\'), 2)');
    this.validateIdentity('SELECT EXP(1)');
    this.validateIdentity('ALTER TABLE table_name ALTER COLUMN bla TYPE VARCHAR');
    this.validateIdentity('SELECT CAST(value AS FLOAT(8))');
    this.validateIdentity('1 div', '1 AS div');
    this.validateIdentity('LISTAGG(DISTINCT foo, \', \')');
    this.validateIdentity('CREATE MATERIALIZED VIEW orders AUTO REFRESH YES AS SELECT 1');
    this.validateIdentity('SELECT DATEADD(DAY, 1, \'today\')');
    this.validateIdentity('SELECT * FROM #x');
    this.validateIdentity('SELECT INTERVAL \'5 DAY\'');
    this.validateIdentity('foo$');
    this.validateIdentity('CAST(\'bla\' AS SUPER)');
    ...
    const expr = this.validateIdentity(
...
ORDER BY
  orderkey_index`,
    ...
    this.validateIdentity('SELECT JSON_PARSE(\'[]\')');
...
FROM (
  ...
  SELECT
    ...
    this.validateIdentity('INSERT INTO t (a) VALUES (1), (2), (3)');
    this.validateIdentity('INSERT INTO t (a, b) VALUES (1, 2), (3, 4)');
    ...
    this.validateIdentity('CREATE TABLE table_backup BACKUP YES AS SELECT * FROM event');
    this.validateIdentity('CREATE TABLE table_backup (i INTEGER, b VARCHAR) BACKUP NO');
    this.validateIdentity('CREATE TABLE table_backup (i INTEGER, b VARCHAR) BACKUP YES');
    this.validateIdentity(
  ...
  testAlterTable () {
    this.validateIdentity('ALTER TABLE s.t ALTER SORTKEY (c)');
    this.validateIdentity('ALTER TABLE t ALTER SORTKEY AUTO');
    this.validateIdentity('ALTER TABLE t ALTER SORTKEY NONE');
    this.validateIdentity('ALTER TABLE t ALTER SORTKEY (c1, c2)');
    this.validateIdentity('ALTER TABLE t ALTER SORTKEY (c1, c2)');
    this.validateIdentity('ALTER TABLE t ALTER COMPOUND SORTKEY (c1, c2)');
    this.validateIdentity('ALTER TABLE t ALTER DISTSTYLE ALL');
    this.validateIdentity('ALTER TABLE t ALTER DISTSTYLE EVEN');
    this.validateIdentity('ALTER TABLE t ALTER DISTSTYLE AUTO');
    this.validateIdentity('ALTER TABLE t ALTER DISTSTYLE KEY DISTKEY c');
    this.validateIdentity('ALTER TABLE t SET TABLE PROPERTIES (\'a\' = \'5\', \'b\' = \'c\')');
    this.validateIdentity('ALTER TABLE t SET LOCATION \'s3://bucket/folder/\'');
    this.validateIdentity('ALTER TABLE t SET FILE FORMAT AVRO');
    this.validateIdentity(
  ...
  testColumnUnnesting () {
    this.validateIdentity('SELECT c.*, o FROM bloo AS c, c.c_orders AS o');
    this.validateIdentity(
  ...
  testGrant () {
    ...
    this.validateIdentity('GRANT ALL ON SCHEMA qa_tickit TO GROUP qa_users');
    this.validateIdentity('GRANT ALL ON TABLE qa_tickit.sales TO GROUP qa_users');
    this.validateIdentity(
      'GRANT ALL ON TABLE qa_tickit.sales TO GROUP qa_users, GROUP ro_users',
    );
    this.validateIdentity('GRANT ALL ON view_date TO view_user');
    this.validateIdentity(
    ...
    this.validateIdentity('GRANT USAGE ON DATABASE sales_db TO Bob');
    this.validateIdentity('GRANT USAGE ON SCHEMA sales_schema TO ROLE Analyst_role');
    this.validateIdentity('GRANT SELECT ON sales_db.sales_schema.tickit_sales_redshift TO Bob');
  }
  ...
  testRevoke () {
    ...
    this.validateIdentity('REVOKE ALL ON SCHEMA qa_tickit FROM GROUP qa_users');
    this.validateIdentity('REVOKE USAGE ON DATABASE sales_db FROM Bob');
    this.validateIdentity('REVOKE USAGE ON SCHEMA sales_schema FROM ROLE Analyst_role');
  }

  testAnalyze () {
    this.validateIdentity('ANALYZE TBL(col1, col2)');
    this.validateIdentity('ANALYZE VERBOSE TBL');
    this.validateIdentity('ANALYZE TBL PREDICATE COLUMNS');
    this.validateIdentity('ANALYZE TBL ALL COLUMNS');
  }
...
describe('TestRedshift', () => {
  ...
  test('testValues', () => t.testValues());
  test('testCreateTableLike', () => t.testCreateTableLike());
  test('testAlterTable', () => t.testAlterTable());
  test('testVarcharMax', () => t.testVarcharMax());
  test('testNoSchemaBinding', () => t.testNoSchemaBinding());
  test('testColumnUnnesting', () => t.testColumnUnnesting());
  test('testJoinMarkers', () => t.testJoinMarkers());
