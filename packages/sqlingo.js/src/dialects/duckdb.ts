...
function lastDaySql (this: Generator, expression: LastDayExpr): string {
  ...
  if (!unit || unit === 'MONTH') {
    return this.func('LAST_DAY', [dateExpr]);
  }
...
function handleNanosecondDiff (this: Generator, endTime: Expression, startTime: Expression): string {
  ...
  return this.sql(
    new SubExpr({
      this: this.func('EPOCH_NS', [endNs]),
      expression: this.func('EPOCH_NS', [startNs]),
    }),
  );
}
  ...
  return function (this: Generator, expression: DatetimeSubExpr | DatetimeAddExpr): string {
    ...
    if (isNanosecondUnit(unit instanceof Expression ? unit : undefined)) {
      ...
      return this.sql(
        this.func('MAKE_TIMESTAMP_NS', [
          new AddExpr({
            this: this.func('EPOCH_NS', [timestampNs]),
    ...
    if (isType(intervalValue, DataTypeExpr.REAL_TYPES)) {
      expression.setArgKey('expression', new CastExpr({
        this: this.func('ROUND', [intervalValue]),
        to: new DataTypeExpr({
...
function weekUnitToDow (unit: Expression | undefined): number | undefined {
  if (unit instanceof VarExpr && 'ISOWEEK'.includes(unit.name.toUpperCase())) {
    return 1;
  }
...
function dayNavigationSql (this: Generator, expression: NextDayExpr | PreviousDayExpr): string {
  ...
  if (expression instanceof NextDayExpr) {
    ...
    const innerExpr = paren(targetDow.sub(isodowCall).add(6), {
      copy: false,
    });
    ...
    const innerExpr = paren(isodowCall.sub(targetDow).add(6), {
      copy: false,
    });
    const daysOffset = new ModExpr({
      this: innerExpr,
      ...
      if (0 < currentGroup.length) {
        if (isWsGroup) {
          currentGroup.forEach((c) =>
            sqlSegments.push(this.func('CHR', [LiteralExpr.number(WS_CONTROL_CHARS_TO_DUCK[c])])));
        } else {
...
function scaleRoundingSql (
  ...
  if (!decimals || expression.args.to) {
    return undefined;
  }
  ...
  const nInt = (decimals.isInteger || decimals.isType([...DataTypeExpr.INTEGER_TYPES]))
    ? decimals
    : new CastExpr({
      this: decimals,
      to: new DataTypeExpr({
  ...
  static get ORIGINAL_TRANSFORMS (): Map<typeof Expression, (this: Generator, e: any) => string> {
    ...
    const transforms = new Map<typeof Expression, (this: Generator, e: any) => string>([
      ...
      [
        ByteLengthExpr,
        function (this: Generator, e) {
          return this.func('OCTET_LENGTH', [e.args.this]);
        },
      ],
      [
        CommentColumnConstraintExpr,
      ...
      [
        ...
        function (this: Generator, e) {
          return e.args.abbreviated
            ...
            : this.func('MONTHNAME', [e.args.this]);
        },
      ],
      [
        DataTypeExpr,
      ...
      [
        ...
        function (this: Generator, e) {
          return this.sql(new EqExpr({
            this: this.func('JSON_TYPE', [e.args.this]),
      ...
      [
        LogicalOrExpr,
        function (this: Generator, e) {
          return this.func('BOOL_OR', [castToBoolean(e.args.this)]);
        },
      ],
      [
        LogicalAndExpr,
        function (this: Generator, e) {
          return this.func('BOOL_AND', [castToBoolean(e.args.this)]);
        },
      ],
      [
        Seq1Expr,
      ...
      [
        UnixMicrosExpr,
        function (this: Generator, e) {
          return this.func('EPOCH_US', [implicitDatetimeCast(e.args.this)]);
        },
      ],
      [
        UnixMillisExpr,
        function (this: Generator, e) {
          return this.func('EPOCH_MS', [implicitDatetimeCast(e.args.this)]);
        },
      ],
      [
        UnixSecondsExpr,
        function (this: Generator, e) {
          return this.sql(
            new CastExpr({
              this: this.func('EPOCH', [implicitDatetimeCast(e.args.this)]),
              to: new DataTypeExpr({
      ...
      [
        ...
        function (this: Generator, e) {
          return this.func('STRFTIME', [
            this.func('TO_TIMESTAMP', [e.args.this]),
            this.formatTime(e),
          ]);
        },
      ],
...
`);
  ...
  static get ARRAYS_ZIP_TEMPLATE () {
    return maybeParse(`
  CASE WHEN :nullCheck THEN NULL
  WHEN :allEmptyCheck THEN [:emptyStruct]
  ELSE LIST_TRANSFORM(RANGE(0, :maxLen), __i -> :transformStruct)
  END
`);
  }
  ...
  bitmapConstructAggSql (expression: BitmapConstructAggExpr): string {
    ...
    return `(${this.sql(replacePlaceholders((this._constructor as typeof DuckDBGenerator).BITMAP_CONSTRUCT_AGG_TEMPLATE.copy(), [], {
      arg,
    }))})`;
  }
  ...
  randstrSql (expression: RandstrExpr): string {
    ...
    return `(${this.sql(replacePlaceholders((this._constructor as typeof DuckDBGenerator).RANDSTR_TEMPLATE.copy(), [], {
      seed: seedValue,
      length,
    }))})`;
  }
  ...
  greatestLeastSql (expression: GreatestExpr | LeastExpr): string {
    ...
    const caseExpr = new CaseExpr({}).when(
      or(allArgs.map((arg) => (arg ?? null_()).is(null_())), {
        copy: false,
      }),
      null_(),
    );
  ...
  normalSql (expression: NormalExpr): string {
    ...
    if (gen instanceof RandExpr && !gen.args.this) {
      ...
      u1 = replacePlaceholders((this._constructor as typeof DuckDBGenerator).SEEDED_RANDOM_TEMPLATE.copy(), [], {
        seed,
      });
      u2 = replacePlaceholders((this._constructor as typeof DuckDBGenerator).SEEDED_RANDOM_TEMPLATE.copy(), [], {
        seed: new AddExpr({
          this: seed.copy(),
          expression: LiteralExpr.number(1),
        }),
    ...
    return this.sql(replacePlaceholders((this._constructor as typeof DuckDBGenerator).NORMAL_TEMPLATE.copy(), [], {
      mean,
      stddev,
      u1,
      u2,
  ...
  timeFromPartsSql (expression: TimeFromPartsExpr): string {
    ...
    if (overflow) {
      ...
      if (nano) {
        sec = (sec as Expression).add(nano.pop().div(LiteralExpr.number('1000000000.0')));
      }

      const totalSeconds = (hour as Expression).mul(LiteralExpr.number(3600))
        .add((minute as Expression).mul(LiteralExpr.number(60)))
        .add(sec as Expression);
  ...
  arraysZipSql (expression: ArraysZipExpr): string {
    ...
    const emptyStruct = new StructExpr({
      expressions: args.map((_, i) => new PropertyEqExpr({
    ...
    const transformStruct = new StructExpr({
      expressions: args.map((arg, i) => new PropertyEqExpr({
        this: LiteralExpr.string(`$${i + 1}`),
    ...
    const nullCheck = or(args.map((arg) => arg.is(null_())));
    const allEmptyCheck = and(args.map((arg) => new EqExpr({
      this: new LengthExpr({
        this: arg,
      }),
      expression: LiteralExpr.number(0),
  ...
  mapPickSql (expression: MapPickExpr): string {
    ...
    if (keysToPick.length === 1 && keysToPick[0].isType?.(DataTypeExprKind.ARRAY)) {
      ...
      lambdaExpr = new LambdaExpr({
        this: new InExpr({ this: xDotKey, expressions: keysToPick }),
        expressions: [toIdentifier('x')],
      });
    }
  ...
  hexStringSql (expression: HexStringExpr): string {
    return super.hexStringSql(expression, {
      binaryFunctionRepr: 'UNHEX',
    });
  }
  ...
  roundSql (expression: RoundExpr): string {
    ...
    if (decimals && expression.args.castsNonIntegerDecimals) {
      if (!(decimals.isInteger || decimals.isType([...DataTypeExpr.INTEGER_TYPES]))) {
