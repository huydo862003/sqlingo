...
export class TypeAnnotator {
  ...
  overwriteTypes: boolean;
  expressionMetadata: ExpressionMetadata;
  coercesTo: Map<DataTypeExprKind | string, Set<DataTypeExprKind | string>>;
  ...
  constructor (options: {
    ...
    if (coercesTo) {
      ...
      if (dialectCoercesTo instanceof Map && 0 < dialectCoercesTo.size) {
        this.coercesTo = dialectCoercesTo;
      } else if (!(dialectCoercesTo instanceof Map) && 0 < Object.keys(dialectCoercesTo).length) {
        this.coercesTo = new Map(
          Object.entries(dialectCoercesTo).map(([
            k,
            v,
          ]) => [
            k as DataTypeExprKind,
            new Set(v as Iterable<DataTypeExprKind>),
          ]),
        );
      } else {
  ...
  annotateScope (scope: Scope): void {
      ...
      for (const tableColumn of scope.tableColumns) {
        ...
        if (source instanceof TableExpr) {
          ...
          const structType = new DataTypeExpr({
            ...
            expressions: Object.entries(schema).map(([
              c,
              kind,
            ]) =>
              new ColumnDefExpr({
    ...
    if (this.dialect._constructor.QUERY_RESULTS_ARE_STRUCTS && scope.expression instanceof QueryExpr) {
      ...
      if (!structType.args.expressions?.some((cd) => {
        const kind = (cd as ColumnDefExpr).args.kind as string | DataTypeExpr | undefined;

        if (!kind) return true;
        if (typeof kind === 'string') return kind === DataTypeExprKind.UNKNOWN;

        return kind.isType(DataTypeExprKind.UNKNOWN);
      })) {
    ...
    for (const [
      ...
      if (expression instanceof UdtfExpr && !(expression instanceof TableFromRowsExpr)) {
        ...
        if (expression instanceof LateralExpr) {
          ...
          if (firstExpr) {
            values = (firstExpr instanceof Expression
              ? firstExpr.args.expressions
              : []) as Expression[];
          }
        }
  ...
  getSetopColumnTypes (
    ...
    for (const node of setop.walk({
      prune: (n) => !(n instanceof SetOperationExpr) && !(n instanceof SubqueryExpr),
    })) {
  ...
  annotateExpression (expression: Expression, scope?: Scope): void {
    ...
    while (0 < stack.length) {
      ...
      if (scope && expr instanceof ColumnExpr && expr.args['table']) {
        ...
        if ((expr.type as DataTypeExpr | undefined)?.getArgKey('nullable') === false) {
      ...
      const spec = this.expressionMetadata.get(expr._constructor);
      ...
      // Fall back to type-based annotation for binary and unary expressions
      if (expr instanceof BracketExpr) {
  ...
  setType (expression: Expression, targetType?: ColumnDefExpr | DataTypeExpr | DataTypeExprKind | string): Expression {
    ...
    if (
      ...
      while (parent instanceof DotExpr) {
        ...
        if (!next.done) {
          parent.setArgKey('expression', toIdentifier(next.value, {
            quoted: true,
          }));
        }
  ...
  annotateBinary (expression: BinaryExpr): void {
    ...
    if (expression instanceof ConnectorExpr || expression instanceof PredicateExpr) {
    ...
    // IsExpr is always nonnull (comparing for null-ness itself is not null)
    // If both operands are non-null, the result is non-null
    if (expression instanceof IsExpr || (left.meta['nonnull'] === true && right.meta['nonnull'] === true)) {
  ...
  annotateUnary (expression: UnaryExpr): void {
    if (expression instanceof NotExpr) {
      ...
      this.setType(expression, innerType as DataTypeExprKind | DataTypeExpr | ColumnDefExpr | undefined);
    }
  ...
  annotateByArgs (
    ...
    if (array) {
      const elementType = (expression.type as DataTypeExpr | undefined)?.copy() ?? new DataTypeExpr({
  ...
  annotateStructValue (
    ...
    if (nameExpr) {
      return new ColumnDefExpr({
        this: nameExpr,
        kind: kind as DataTypeExpr | undefined,
      });
    }
  ...
  annotateMap (expression: MapExpr | VarMapExpr): void {
    ...
    const mapType = new DataTypeExpr({
    ...
    if (keysExpr instanceof ArrayExpr && valuesExpr instanceof ArrayExpr) {
      ...
      if (keyKind !== DataTypeExprKind.UNKNOWN && valueKind !== DataTypeExprKind.UNKNOWN) {
        mapType.setArgKey('expressions', [
          keyTypeExpr ?? new DataTypeExpr({
            this: DataTypeExprKind.UNKNOWN,
          }),
          valueTypeExpr ?? new DataTypeExpr({
            this: DataTypeExprKind.UNKNOWN,
          }),
        ]);
        mapType.setArgKey('nested', true);
      }
    }
  ...
  annotateToMap (expression: ToMapExpr): void {
    ...
    if (arg?.isType(DataTypeExprKind.STRUCT) && arg.type instanceof Expression) {
        ...
        if (kindKind !== DataTypeExprKind.UNKNOWN) {
          ...
          mapType.setArgKey('expressions', [
            ...
            kind instanceof DataTypeExpr
              ? kind
              : new DataTypeExpr({
                this: kindKind ?? DataTypeExprKind.UNKNOWN,
              }),
          ]);
