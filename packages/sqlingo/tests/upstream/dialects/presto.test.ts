...
import {
  DataTypeExpr, DataTypeExprKind, LiteralExpr, TruncExpr,
} from '../../../src/expressions';
import {
  Validator,
} from './validator';

class TestPresto extends Validator {
  override dialect = 'presto' as const;

  testCast () {
    this.validateIdentity('DEALLOCATE PREPARE my_query', undefined, {
      checkCommandWarning: true,
    });
    this.validateIdentity('DESCRIBE INPUT x', undefined, {
      checkCommandWarning: true,
    });
    this.validateIdentity('DESCRIBE OUTPUT x', undefined, {
      checkCommandWarning: true,
    });
    this.validateIdentity('SELECT * FROM x qualify', 'SELECT * FROM x AS qualify');
    this.validateIdentity('CAST(x AS IPADDRESS)');
    this.validateIdentity('CAST(x AS IPPREFIX)');
    this.validateIdentity('CAST(TDIGEST_AGG(1) AS TDIGEST)');
    this.validateIdentity('CAST(x AS HYPERLOGLOG)');
    this.validateIdentity('RESET SESSION hive.optimized_reader_enabled', undefined, {
      checkCommandWarning: true,
    });
  ...
  testIntervalPluralToSingular () {
    ...
    for (const [
      unit,
      expected,
    ] of Object.entries(unitToExpected)) {
      this.validateAll(
        `SELECT INTERVAL '1' ${unit}`,
        {
          write: {
            bigquery: `SELECT INTERVAL '1' ${expected}`,
            presto: `SELECT INTERVAL '1' ${expected}`,
            trino: `SELECT INTERVAL '1' ${expected}`,
            mysql: `SELECT INTERVAL '1' ${expected}`,
            doris: `SELECT INTERVAL '1' ${expected}`,
          },
        },
      );
    }
  ...
  testTime () {
    const expr = parseOne('TIME(7) WITH TIME ZONE', {
      into: DataTypeExpr,
      read: 'presto',
    });

    expect(expr.getArgKey('this')).toBe(DataTypeExprKind.TIMETZ);

    this.validateIdentity('FROM_UNIXTIME(a, b)');
    this.validateIdentity('FROM_UNIXTIME(a, b, c)');
    this.validateIdentity('TRIM(a, b)');
    this.validateIdentity('VAR_POP(a)');
    ...
    this.validateIdentity('DATE_ADD(\'DAY\', 1, y)');
    ...
    this.validateIdentity('DATE_ADD(\'DAY\', FLOOR(5), y)');
    this.validateIdentity(
  ...
  testDdl () {
    ...
    this.validateIdentity('CREATE OR REPLACE VIEW v SECURITY DEFINER AS SELECT id FROM t');
    this.validateIdentity('CREATE OR REPLACE VIEW v SECURITY INVOKER AS SELECT id FROM t');
  }
  ...
  testUnicodeString () {
    for (const prefix of [
      ...
      this.validateAll(
        `${prefix}'Hello winter #2603 !' UESCAPE '#'`,
        {
          write: {
            oracle: 'U\'Hello winter \\2603 !\'',
  ...
  testPresto () {
    expect(
      func('md5', func('concat', cast('x', 'text'), LiteralExpr.string('s'))).sql(
        {
          dialect: 'presto',
        },
      ),
    ...
    this.validateIdentity('SELECT a FROM t GROUP BY a, ROLLUP (b), ROLLUP (c), ROLLUP (d)');
    this.validateIdentity('SELECT a FROM test TABLESAMPLE BERNOULLI (50)');
    this.validateIdentity('SELECT a FROM test TABLESAMPLE SYSTEM (75)');
    this.validateIdentity('string_agg(x, \',\')', 'ARRAY_JOIN(ARRAY_AGG(x), \',\')');
    this.validateIdentity('SELECT * FROM x OFFSET 1 LIMIT 1');
    this.validateIdentity('SELECT * FROM x OFFSET 1 FETCH FIRST 1 ROWS ONLY');
    this.validateIdentity('SELECT BOOL_OR(a > 10) FROM asd AS T(a)');

    // Numeric TRUNCATE
    this.validateIdentity('TRUNCATE(3.14159, 2)').assertIs(TruncExpr);
    this.validateIdentity('TRUNCATE(3.14159)').assertIs(TruncExpr);
    this.validateAll(
      'TRUNCATE(3.14159, 2)',
      {
        read: {
    ...
    this.validateIdentity('SELECT * FROM (VALUES (1))');
    this.validateIdentity('START TRANSACTION READ WRITE, ISOLATION LEVEL SERIALIZABLE');
    this.validateIdentity('START TRANSACTION ISOLATION LEVEL REPEATABLE READ');
    this.validateIdentity('APPROX_PERCENTILE(a, b, c, d)');
    this.validateIdentity(
    ...
    this.validateAll('VALUES 1, 2, 3', {
      write: {
        presto: 'VALUES (1), (2), (3)',
      },
    });
    this.validateAll('INTERVAL \'1 day\'', {
      write: {
        trino: 'INTERVAL \'1\' DAY',
      },
    });
    this.validateAll('(5 * INTERVAL \'7\' DAY)', {
      read: {
        '': 'INTERVAL \'5\' WEEK',
      },
    });
  ...
  testEncodeDecode () {
    this.validateIdentity('FROM_UTF8(x, y)');
  ...
  testJsonVsRowExtract () {
    for (const dialect of [
      'trino',
      'presto',
    ] as const) {
      const s = parseOne('SELECT col:x:y."special string"', {
        read: 'snowflake',
      });
      ...
      expect(s.sql({
        dialect,
      })).toBe(jsonExtractResult);
      expect(s.sql({
        dialect: dialectJsonExtractSetting,
      })).toBe(jsonExtractResult);

      // If the setting is overriden to False, then generate ROW access (dot notation)
      expect(s.sql({
        dialect: dialectRowAccessSetting,
      })).toBe('SELECT col.x.y."special string"');
    }
  }
...
describe('TestPresto', () => {
  ...
  test('test_interval_plural_to_singular', () => {
    presto.testIntervalPluralToSingular();
  });
  test('test_time', () => {
    presto.testTime();
  });
  ...
  test('test_unicode_string', () => {
    presto.testUnicodeString();
  });
  test('test_presto', () => {
    presto.testPresto();
  });
  test('test_encode_decode', () => {
    presto.testEncodeDecode();
  });
  test('test_hex_unhex', () => {
    presto.testHexUnhex();
  });
  ...
  test('test_match_recognize', () => {
    presto.testMatchRecognize();
  });
  test('test_to_char', () => {
    presto.testToChar();
  });
  ...
  test('test_json_vs_row_extract', () => {
    presto.testJsonVsRowExtract();
  });
  test('test_analyze', () => {
    presto.testAnalyze();
  });
